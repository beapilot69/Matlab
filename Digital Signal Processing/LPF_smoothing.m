    ...
    for col = (mask_len-1)/2 + 1 : col_size + (mask_len-1)/2
        if col < mask_len         % 열이 차수보다 작다면 -> 다음의 계산과정 필요
            ...
            if (col <= 256)
                ...
                for k=1:mask_len-(col-col_size)
                    temp = temp + double(x(row,col_size+1-k));
                end
                y(row,col-(mask_len-1)/2) = temp / mask_len;
            end
        end
...
%% 최종 출력 y를 이미지로 변환
imwrite(y,map,'lena_LPF_3.jpg'); 

%% FFT
n= length(x);
f= (-n/2:n/2-1)*(200/n);

subplot(2,1,1);plot(f,abs(fftshift(fft(x(200,1:end)))));
subplot(2,1,2);plot(f,abs(fftshift(fft(y(200,1:end)))));
